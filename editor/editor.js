  ...
  once &&
    once("editor-init", () => {
      ...
      const app = {
        ...
        lyricsEditorContainer: document.getElementById(
        ...
        setupEventListeners() {
          ...
          document
            ...
            ?.addEventListener("click", () => {
              if (!this.currentSong) return;
              const format = prompt(
                'Export format? Enter "json" or "txt"',
                "txt",
              );
              if (!format) return;
              let blob, filename;
        ...
        updateSongMetadata() {
          ...
          const tagsValue = tagsEl?.value
            ...
            .filter((t) => t);
          if (
            JSON.stringify(this.currentSong.tags) !== JSON.stringify(tagsValue)
          ) {
        ...
        renderLyrics() {
          ...
          const rhymeGroups = this.isRhymeMode
            ? this.findRhymes(lyricLines)
            : {};
        ...
        insertSectionAtCursor(label) {
          ...
          let lineGroup = null;
          if (currentSection) {
            this.lyricsDisplay.insertBefore(
              section,
        ...
        updateReadOnlyState() {
          ...
          this.lyricsDisplay.querySelectorAll(".lyric-text").forEach((line) => {
            const editable =
              !isReadOnly && (mode === "lyrics" || mode === "both");
          ...
          this.lyricsDisplay.querySelectorAll(".chord-line").forEach((line) => {
            const editable =
              !isReadOnly && (mode === "chords" || mode === "both");
            line.setAttribute("contenteditable", editable);
            line.classList.toggle("editable", editable);
            line.classList.toggle("non-editable", !editable);
          });
      ...
      document
        .getElementById("copy-lyrics-btn")
        ?.addEventListener("click", () => {
      ...
      document
        .getElementById("editor-menu-btn")
        ?.addEventListener("click", () => {
      ...
      document
        .querySelectorAll("#ai-tools-modal .tool-option")
        ?.forEach((btn) => {
          btn.addEventListener("click", () => {
