document.addEventListener("DOMContentLoaded", () => {
  const { safeParse, log, once } = (window.App && window.App.Utils) || {
    safeParse: (s) => {
      try {
        return JSON.parse(s);
      } catch {
  ...
  const app = {
    ...
    closeAutoscrollDelayModal: document.getElementById(
      "close-autoscroll-delay-modal",
    ),
    ...
    stripTitleLine(lyrics, title) {
      return App.Utils.stripTitleLine(lyrics, title);
    },
    splitChordLyric(lyrics = "", chords = "") {
      return App.Utils.splitChordLyric(lyrics, chords);
    },
    compactBlankLines(text = "") {
      return App.Utils.compactBlankLines(text);
    },

    // Initialize
    init() {
      this.loadData();
      if (this.perfEditModeSelect)
        this.perfEditModeSelect.value = this.editMode;
      if (this.perfChordModeSelect)
        this.perfChordModeSelect.value = this.chordMode;
    ...
    loadPerformanceState() {
      ...
      const songId =
        params.get("songId") ||
        localStorage.getItem("hrsm:currentSongId") ||
        null;
      this._explicitSongParam = params.has("songId");
      try {
        console.debug(
          "[Performance load] setlistId=",
      ...
      if (this.performanceSetlistId) {
        ...
        if (setlistRaw) {
          ...
          const setlist = setlists.find(
            (s) => String(s.id) === String(this.performanceSetlistId),
          );
          if (setlist) {
      ...
      this.currentPerformanceSongIndex = songId
        ? this.performanceSongs.findIndex(
    ...
    setupEventListeners() {
      ...
      this.footerDecreaseFontBtn?.addEventListener("click", () =>
        this.adjustFontSize(-this.fontSizeStep),
      );
      this.footerIncreaseFontBtn?.addEventListener("click", () =>
        this.adjustFontSize(this.fontSizeStep),
      );

      this.toggleThemeBtn?.addEventListener("click", () =>
        this.handlePerformanceThemeToggle(),
      );
      this.exitPerformanceBtn?.addEventListener("click", () =>
      ...
      this.closeAutoscrollDelayModal?.addEventListener("click", () => {
        ...
        if (song && song.id) {
          ...
          localStorage.setItem(
            "perSongAutoScroll",
            JSON.stringify(this.perSongAutoScroll),
          );
        }
      ...
      this.lyricsDisplay.addEventListener("touchstart", () =>
        this.stopAutoScroll(),
      );
      this.lyricsDisplay.addEventListener("mousedown", () =>
        this.stopAutoScroll(),
      );
      ...
      this.perfMenuBtn?.addEventListener("click", () =>
        this.perfMenuModal.classList.add("is-open"),
      );
      this.perfMenuClose?.addEventListener("click", () =>
        this.perfMenuModal.classList.remove("is-open"),
      );
      ...
      document.addEventListener("keydown", (e) => {
        if (e.key === "Escape") {
          if (this.autoscrollDelayModal?.classList.contains("is-open"))
            this.autoscrollDelayModal.classList.remove("is-open");
          if (this.perfMenuModal?.classList.contains("is-open"))
            this.perfMenuModal.classList.remove("is-open");
          if (this.perfMetadataModal?.classList.contains("is-open"))
            this.perfMetadataModal.classList.remove("is-open");
        }
      });
    },
    ...
    displayCurrentPerformanceSong() {
      ...
      if (!song) {
        this.lyricsDisplay.innerHTML =
          '<p class="empty-state">Pick a song to see performance tips.</p>';
      ...
      this.prevSongBtn.style.display =
        this.currentPerformanceSongIndex > 0 ? "block" : "none";
      this.nextSongBtn.style.display =
        this.currentPerformanceSongIndex < this.performanceSongs.length - 1
          ? "block"
          : "none";
      ...
      if (auto) {
        ...
        this.autoscrollDelay =
          Number(localStorage.getItem("autoscrollDelay")) || 3;
        this.autoScrollSpeed =
          Number(localStorage.getItem("autoscrollSpeed")) || 1;
      }
    ...
    renderLyrics() {
      ...
      const lyricsNorm = App.Utils.normalizeSectionLabels(
        this.cleanText(this.stripTitleLine(song.lyrics || "", title)),
      );
      const chordsClean = this.compactBlankLines(
        (song.chords || "").replace(/\r\n/g, "\n"),
      );
      ...
      for (let i = 0; i < L.length; i++) {
        ...
        chordEl.style.display =
          mode === "off" || mode === "lyrics" ? "none" : "block";
    ...
    applyEditMode() {
      ...
      if (this.editMode === "lyrics" || this.editMode === "both") {
        container
          ...
          .forEach((n) => n.setAttribute("contenteditable", "true"));
      }
    ...
    persistEditsFromDOM() {
      ...
      const chordLines = [];
      this.lyricsDisplay
        .querySelectorAll(".section, .lyrics-line-group")
        .forEach((node) => {
          if (node.classList.contains("section")) {
            const label =
              node.querySelector(".section-label-text")?.textContent || "";
            ...
            node.querySelectorAll(".lyrics-line-group").forEach((group) => {
              ...
              const lyric =
                group.querySelector(".lyric-text")?.textContent ?? "";
      ...
      let lyricsOut = App.Utils.normalizeSectionLabels(
        this.compactBlankLines(lyricLines.join("\n")),
      );
      ...
      if (idx !== -1) {
        all[idx] = song;
        localStorage.setItem(App.Config.STORAGE.SONGS, JSON.stringify(all));
      }
    },
    ...
    normalizeCurrentSong() {
      const song = this.performanceSongs[this.currentPerformanceSongIndex];
      if (!song) return;
      let L = this.stripTitleLine(
        this.cleanText(song.lyrics || ""),
        song.title || "",
      );
      L = App.Utils.normalizeSectionLabels(this.compactBlankLines(L));
      let C = this.compactBlankLines(
        (song.chords || "").replace(/\r\n/g, "\n"),
      );
      const Ls = L.split("\n");
      const Cs = C.split("\n");
      const max = Math.max(Ls.length, Cs.length);
      while (Ls.length < max) Ls.push("");
      while (Cs.length < max) Cs.push("");
      song.lyrics = Ls.join("\n");
      song.chords = Cs.join("\n");
      song.lastEditedAt = new Date().toISOString();
      const all = safeParse(localStorage.getItem(App.Config.STORAGE.SONGS), []);
    ...
    updateHeaderMetaLine() {
      ...
      if (song.timeSignature && song.timeSignature !== "4/4")
        metaBits.push(song.timeSignature);
    ...
    adjustFontSize(amount) {
      this.fontSize = Math.max(
        this.minFontSize,
        Math.min(this.maxFontSize, this.fontSize + amount),
      );
      this.updateFontSize();
    ...
    updateAutoScrollButton() {
      ...
      btn.title = this.autoScrollActive
        ? "Pause Autoscroll"
        : "Start Autoscroll";
    },
